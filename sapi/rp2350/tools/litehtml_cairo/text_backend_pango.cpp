...
pango_text_backend::pango_text_backend()
{
...
litehtml::uint_ptr pango_text_backend::create_font(const litehtml::font_description& descr, const litehtml::document* doc, litehtml::font_metrics *fm)
{
...
void pango_text_backend::delete_font(litehtml::uint_ptr hFont)
{
...
litehtml::pixel_t pango_text_backend::text_width(const char *text, litehtml::uint_ptr hFont)
{
    auto* fnt = (cairo_font*) hFont;
...
static void draw_solid_line(cairo_t* cr, litehtml::pixel_t x, litehtml::pixel_t y, litehtml::pixel_t width, int thickness, draw_type type, litehtml::web_color& color)
{
    ...
    cairo_set_source_rgba(cr, (double) color.red / 255.0, (double) color.green / 255.0, (double) color.blue / 255.0, (double) color.alpha / 255.0);
...
static void draw_wavy_line(cairo_t* cr, litehtml::pixel_t x, litehtml::pixel_t y, litehtml::pixel_t width, int thickness, draw_type type, litehtml::web_color& color)
{
    ...
    cairo_set_source_rgba(brush_cr, (double) color.red / 255.0, (double) color.green / 255.0, (double) color.blue / 255.0, (double) color.alpha / 255.0);
...
void pango_text_backend::draw_text(litehtml::uint_ptr hdc, const char *text, litehtml::uint_ptr hFont, litehtml::web_color color, const litehtml::position &pos)
{
    auto* fnt = (cairo_font*) hFont;
    auto* cr = (cairo_t*) hdc;
    cairo_save(cr);
    ...
    auto draw_func = draw_funcs[fnt->decoration_style];
    if(fnt->underline)
        draw_func(cr, x, y + text_baseline + fnt->underline_position, tw, fnt->underline_thickness, draw_type::DRAW_UNDERLINE, decoration_color);
    if(fnt->overline)
        draw_func(cr, x, y + text_baseline - fnt->overline_position, tw, fnt->overline_thickness, draw_type::DRAW_OVERLINE, decoration_color);
    if(fnt->strikeout)
        draw_func(cr, x, y + text_baseline - fnt->strikethrough_position, tw, fnt->strikethrough_thickness, draw_type::DRAW_STRIKETHROUGH, decoration_color);
