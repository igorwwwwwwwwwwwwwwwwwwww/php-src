...
- `main.php` and each file under `fs/` are embedded into flash at build time via a recursive VFS builder; nested asset directories work.
...
- With `RP2350_ENABLE_EPD=OFF`, the full Tufty PHP firmware fits in flash and SRAM.
...
### On-device HTML rendering -- investigated, not viable

A full investigation of on-device HTML rendering was conducted using litehtml. Summary:

- litehtml compiles and links into the firmware successfully
- the firmware boots and reaches the render call
- litehtml's document tree uses ~3MB for a 5KB stripped HTML snippet (shared_ptr control blocks per node, STL containers throughout)
- PSRAM layout is: 2MB newlib heap + Zend mmap arena (needs ≥4MB for 2MB-aligned 2MB chunks) + firmware = no room for a 3MB+ renderer arena
- C++ static globals in litehtml also allocate before `main()` / before PSRAM is ready; this is solvable with a pre-PSRAM SRAM arena override for `operator new`, but is a further sign of how allocation-heavy the library is

**Conclusion:** not viable on this hardware with litehtml. Future options:
- A purpose-built renderer with fixed-size node pools (no `shared_ptr`, no per-node STL containers)
- Server-side pre-rendering (fetch a bitmap or layout description from a proxy)
- Render on host, push compressed framebuffer over Wi-Fi
