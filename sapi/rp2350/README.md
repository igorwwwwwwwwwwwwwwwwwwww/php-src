...
- Fix error output mode for MCU console:
  - Runtime still emits HTML-style `<br />`/`<b>` formatting in fatal paths.
  - Existing INI defaults intended to force CLI-style plain text (`html_errors=0`, etc.) are not consistently taking effect.
  - Trace where startup/runtime error formatting overrides are coming from and enforce plain-text output.
